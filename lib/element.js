var DOMException = require('./dom-exception')
, Node = require('./node')
;

class NodeList {

  constructor(head, tail) {
    this._head = head || null;
    this._tail = tail || null;
  }

}

class Element extends Node {

  constructor(tagName, parent) {
    super(parent);

    this._name = tagName;
  }

  get tagName()    { return this._name; }
  get nodeName()   { return this.tagName; }
  get nodeType()   { return Node.ELEMENT_NODE; }

  get firstChild() { return this._children ? this._children._head : null; }
  get lastChild()  { return this._children ? this._children._tail : null; }

  insertBefore(newChild, beforeChild) {
    if (this._isAncestorOrSelf(newChild))
      throw new DOMException(DOMException.HIERARCHY_REQUEST_ERR);

    if (newChild.parentNode)
      newChild.parentNode.removeChild(newChild);

    if (beforeChild && ! this._isChildNode(beforeChild))
      throw new DOMException(DOMException.NOT_FOUND_ERR);

    if (! this._children)
      this._children = new NodeList();

    if (! beforeChild) {

      newChild._prev = this._children._tail;
      this._children._tail = newChild;

    } else {

      newChild._prev = beforeChild.previousSibling;
      newChild._next = beforeChild;
      beforeChild._prev = newChild;

    }

    if (newChild._prev)
      newChild._prev._next = newChild;
    else
      this._children._head = newChild;

    newChild._parent = this;

    return newChild;
  }


  replaceChild(withChild, child) {
    this.insertBefore(withChild, child);

    return this.removeChild(child);
  }

  removeChild(child) {
    if (! this._isChildNode(child))
      throw new DOMException(DOMException.NOT_FOUND_ERR);

    if (child._prev)
      child._prev._next = child.nextSibling;
    else
      this._children._head = child.nextSibling;

    if (child._next)
      child._next._prev = child.previousSibling;
    else
      this._children._tail = child.previousSibling;

    child._parent = null;
    child._next = null;
    child._prev = null;

    return child;
  }

  appendChild(child) {
    this.insertBefore(child);
  }

  hasChildNodes() {
    if (this._children && this._children._head)
      return true;

    return false;
  }

  cloneNode(deep) {
    let clone = new Element(this.tagName);

    if (deep) {
      var child = this.firstChild;

      while (child) {
        clone.appendChild(child.cloneNode(deep));
        child = child.nextSibling;
      }
    }

    return clone;
  }

  _isChildNode(node) {
    var cur = this.firstChild;

    while (cur) {
      if (node === cur)
        return true;

      cur = cur.nextSibling;
    }

    return false;
  }

  _isAncestorOrSelf(node) {
    var cur = this;

    while (cur) {
      if (node === cur)
        return true;

      cur = cur.parentNode;
    }

    return false;
  }
}

module.exports = Element;
